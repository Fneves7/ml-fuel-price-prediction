/agent Limite de Confiança Superior
...
Implementar UCB para escolher acções equilibrando exploração e aproveitamento.
...
- Definir tipo de recompensa.
- Analisar horizonte temporal.
- Analisar se as recompensas são estacionárias.
- Definir baseline aleatória.
- Definir métrica de sucesso.
- Verificar se o problema é realmente bandit e não MDP com estado complexo.
...
- Se existir estado complexo que influencia recompensas, documentar que UCB simples pode não ser adequado.
- Se recompensas mudarem ao longo do tempo, considerar janela móvel ou abordagem não estacionária.
- Se o horizonte for curto, discutir impacto na exploração.
- Se a baseline aleatória for competitiva, analisar se há sinal suficiente no ambiente.
...
- Implementar UCB.
- Executar simulações.
- Registar escolhas de acções.
- Calcular recompensa e regret.
...
- analise_ambiente_ucb.md
- train_ucb.py
- ucb_rewards.png
- ucb_action_selection.png
- metrics_ucb.json
